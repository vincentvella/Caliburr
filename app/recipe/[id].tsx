    ...
    <View className="flex-1 bg-ristretto-900">
      ...
      <ScrollView className="flex-1" contentContainerClassName="px-6 pt-6 pb-24 gap-6">
        ...
        <View className="flex-row flex-wrap gap-3">
          ...
          {recipe.brew_time_s != null && <StatCard label="Brew Time" value={formatTime(recipe.brew_time_s)} />}
          {recipe.water_temp_c != null && <StatCard label="Temp" value={`${recipe.water_temp_c}°C`} />}
          ...
          {recipe.roast_date && (
            <StatCard label="Roast Date" value={recipe.roast_date} />
          )}
        </View>
        ...
        {recipe.notes && (
          <View className="gap-2">
            <Text className="text-latte-400 text-xs font-semibold uppercase tracking-wider">Notes</Text>
        ...
        {/* Equipment */}
        <View className="gap-2">
          <Text className="text-latte-400 text-xs font-semibold uppercase tracking-wider">Equipment</Text>
          <EquipmentRow
            label="Grinder"
            value={grinderLabel}
            subtitle={recipe.grinder.burr_type && recipe.grinder.adjustment_type
          ...
          {recipe.bean && (
            <EquipmentRow
              ...
              subtitle={[recipe.bean.origin, recipe.bean.process].filter(Boolean).join(" · ") || undefined}
            />
          )}
        </View>
        ...
        {isOwner && history.length > 0 && (
          <View className="gap-2">
            <Text className="text-latte-400 text-xs font-semibold uppercase tracking-wider">Edit History</Text>
            {history.map((entry, i) => (
              <HistoryCard key={entry.id} entry={entry} current={i === 0 ? recipe : history[i - 1]} />
            ))}
          </View>
        )}
      </ScrollView>
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <View className="bg-ristretto-800 border border-ristretto-700 rounded-xl px-4 py-3 items-center" style={{ minWidth: 80 }}>
...
type HistorySnapshot = Pick<RecipeHistory,
  'grind_setting' | 'dose_g' | 'yield_g' | 'brew_time_s' | 'water_temp_c' | 'ratio' | 'roast_level' | 'roast_date' | 'notes'
>;
...
function HistoryCard({ entry, current }: { entry: RecipeHistory; current: HistorySnapshot }) {
  ...
  if (current.dose_g !== entry.dose_g)
    diffs.push({ label: "Dose", was: entry.dose_g != null ? `${entry.dose_g}g` : "—", now: current.dose_g != null ? `${current.dose_g}g` : "—" });
  if (current.yield_g !== entry.yield_g)
    diffs.push({ label: "Yield", was: entry.yield_g != null ? `${entry.yield_g}g` : "—", now: current.yield_g != null ? `${current.yield_g}g` : "—" });
  if (current.brew_time_s !== entry.brew_time_s)
    diffs.push({ label: "Brew Time", was: entry.brew_time_s != null ? formatTime(entry.brew_time_s) : "—", now: current.brew_time_s != null ? formatTime(current.brew_time_s) : "—" });
  if (current.water_temp_c !== entry.water_temp_c)
    diffs.push({ label: "Temp", was: entry.water_temp_c != null ? `${entry.water_temp_c}°C` : "—", now: current.water_temp_c != null ? `${current.water_temp_c}°C` : "—" });
  if (current.ratio !== entry.ratio)
    diffs.push({ label: "Ratio", was: entry.ratio != null ? `1:${entry.ratio}` : "—", now: current.ratio != null ? `1:${current.ratio}` : "—" });
  if (current.roast_level !== entry.roast_level)
    diffs.push({ label: "Roast", was: entry.roast_level ? ROAST_LEVEL_LABELS[entry.roast_level] : "—", now: current.roast_level ? ROAST_LEVEL_LABELS[current.roast_level] : "—" });
  if (current.roast_date !== entry.roast_date)
    diffs.push({ label: "Roast Date", was: entry.roast_date ?? "—", now: current.roast_date ?? "—" });
    ...
    >
      <View className="flex-row items-center justify-between">
        <View className="gap-0.5">
          <Text className="text-latte-200 text-sm font-medium">{dateStr} · {timeStr}</Text>
          <Text className="text-latte-500 text-xs">
            {diffs.length === 0 ? "No tracked changes" : `${diffs.length} field${diffs.length === 1 ? "" : "s"} changed`}
          </Text>
        </View>

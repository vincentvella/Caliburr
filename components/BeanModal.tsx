...
export function BeanModal({ visible, onClose, onSelected, selectedId }: Props) {
  ...
  useEffect(() => {
    if (visible) { setView("search"); setQuery(""); setResults([]); }
  }, [visible]);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1 bg-ristretto-900">
        <View className="flex-row items-center justify-between px-6 pt-6 pb-4 border-b border-ristretto-700">
          ...
          <TouchableOpacity onPress={view === "search" ? handleClose : () => setView("search")}>
            <Text className="text-harvest-400 font-semibold">{view === "search" ? "Cancel" : "Back"}</Text>
          </TouchableOpacity>
        </View>
          ...
          <View className="flex-1 px-6 pt-4">
            ...
            {searching ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    onPress={() => { onSelected(item); handleClose(); }}
                  ...
                  >
                    ...
                    {selectedId === item.id && (
                      <Text className="text-harvest-400">✓</Text>
                    )}
                  </TouchableOpacity>
                )}
                ListEmptyComponent={
        ...
        {view === "create" && (
          <BeanForm
            initialName={query}
            onDone={(bean) => { onSelected(bean); handleClose(); }}
          />
        )}
      </KeyboardAvoidingView>
    </Modal>
      ...
      <form.Field name="roast_level">
        {(field) => (
          <View className="gap-2">
            <Text className="text-latte-400 text-xs px-1">Roast Level <Text className="text-latte-600">(optional)</Text></Text>
            <View className="flex-row flex-wrap gap-2">
              {ROAST_LEVELS.map((level) => (
                ...
                >
                  <Text className={`text-xs font-medium ${field.state.value === level ? "text-white" : "text-latte-400"}`}>
